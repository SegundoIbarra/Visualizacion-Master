...
function handleEmptyListError() {
    if (nodes.length === 0) {
        error.innerHTML = errorCircle + " La lista esta vacia";
...
function checkInputErrors(input, type, endsAtLastNode = false) {
    ...
    if (isNaN(input)) {
        error.innerHTML =  errorCircle + type + " Tiene que ser un numero";
        inputError = true;
    }
    else if (type === "Index" && (input > end || input < 0)) {
        error.innerHTML = errorCircle + "Indice fuera de los limites";
        inputError = true;
    }
